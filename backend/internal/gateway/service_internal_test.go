...
func TestPrepareUpstreamExchangeSelectsPathAndResponseModeForBridging(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
	...
	}{
		{
			...
			publicProtocol: model.ProtocolOpenAIChat,
			upstream:       model.ProtocolOpenAIResponses,
			...
			expectMode: responseModeOpenAIChatJSON,
		},
		{
			name:           "openai chat stream to anthropic",
			publicProtocol: model.ProtocolOpenAIChat,
			upstream:       model.ProtocolAnthropic,
			path:           "/v1/chat/completions",
			request: parsedProxyRequest{
				routeAlias: "alias",
				stream:     true,
			...
			expectMode: responseModeOpenAIChatSSE,
		},
		{
			name:           "anthropic stream to responses",
			publicProtocol: model.ProtocolAnthropic,
			upstream:       model.ProtocolOpenAIResponses,
			path:           "/v1/messages",
			request: parsedProxyRequest{
				routeAlias: "alias",
			...
			expectPath: "/v1/responses",
			expectMode: responseModeAnthropicSSE,
		},
		{
			name:           "gemini stream to openai chat",
			publicProtocol: model.ProtocolGeminiStream,
			upstream:       model.ProtocolOpenAIChat,
			path:           "/v1beta/models/alias:streamGenerateContent",
			request: parsedProxyRequest{
				routeAlias: "alias",
				stream:     true,
			...
			expectMode: responseModeGeminiSSE,
		},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			exchange, err := prepareUpstreamExchange(tc.publicProtocol, tc.upstream, tc.path, tc.request, "provider-model")
			if err != nil {
				t.Fatalf("prepare upstream exchange: %v", err)
			}
			if exchange.Path != tc.expectPath {
				t.Fatalf("unexpected path: %s", exchange.Path)
			}
			if exchange.ResponseMode != tc.expectMode {
